...
import net.jeeshop.core.pay.fuiuo.client.config.FuiouConfig;
...
//	private OssService ossService;
	...
	public void loadPlugConfig() {
		...
		pay.setStatus(Pay.PAY_USE_STATUS);
		...
		if(pay != null){
			...
			pays.put(Pay.PAY_CODE_ALIPAY, pay.getName());
		}
		/**
		 * 加载富友支付
		 */
		Pay fuiouPay = new Pay();
		fuiouPay.setCode(Pay.PAY_CODE_FUIOU);
		fuiouPay.setStatus(Pay.PAY_USE_STATUS);
		fuiouPay = payService.selectOne(fuiouPay);
		if(fuiouPay != null){
			pays.put(Pay.PAY_CODE_FUIOU, fuiouPay.getName());
//			systemManager.setAlipayConfig(pay.getSeller());
			FuiouConfig.mchnt_cd = fuiouPay.getPartner();
			FuiouConfig.mchnt_key = fuiouPay.getKey1();
		}
		/**
		 * 加载开联通支付
		 */
		Pay openePay = new Pay();
		openePay.setCode(Pay.PAY_CODE_OPENE);
		openePay.setStatus(Pay.PAY_USE_STATUS);
		openePay = payService.selectOne(openePay);
		if(openePay != null){
			pays.put(Pay.PAY_CODE_OPENE, openePay.getName());
//			systemManager.setAlipayConfig(pay.getSeller());
			OpeneConfig.merchantId = openePay.getPartner();
			OpeneConfig.key = openePay.getKey1();
		}
		systemManager.setPays(pays);
		/**
		 * 加载评论配置
		 */
